...
function injectDevelocityForMaven() {
  local rootDir=$1
  local mavenOpts="-Ddevelocity.scan.uploadInBackground=false -Ddevelocity.allowUntrustedServer=${allowUntrustedServer}"
  ...
  then
    mavenOpts="${mavenOpts} -Ddevelocity.url=${url}"
  ...
  then
      mavenOpts="${mavenOpts} -Ddevelocity.scan.captureFileFingerprints=${captureFileFingerprints}"
  fi
